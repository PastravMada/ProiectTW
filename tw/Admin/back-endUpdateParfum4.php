...
<body>
	...
	if(isset($_POST['nume_parfum'])&&isset($_POST['pret'])&&isset($_POST['stoc'])&&isset($_POST['nume_brand'])&&isset($_POST['categorie'])&&isset($_POST['ocazie'])&&isset($_POST['anotimp'])&&isset($_POST['ingrediente'])&&isset($_POST['descriere'])){
		...
		$stid=oci_parse($conn, 'begin :r := pachet_proiect.return_id_brand(:p); end;');
		oci_bind_by_name($stid, ':p', $nume_brand);
		oci_bind_by_name($stid, ':r', $id_brand,40);
		oci_execute($stid);
	...
	oci_close($conn);
	echo "<h2 style='color:black;'>Datele au fost modificate cu succes</h2>";
