...
if(isset($_GET['denumire'])){
	$valoare=$_GET['denumire'];
	$conn=oci_connect("mihaela","veronica","localhost/XE");
    	if(!$conn)
        	echo ("EROARE LA CONEXIUNEA CU ORACLE DB");
        else{
				$sql='SELECT * FROM PLANTE WHERE DENUMIRE_POPULARA=:valoare OR DENUMIRE_STIINTIFICA=:valoare';
                ...
                oci_execute($stid);            
                    while(oci_fetch($stid)){
						...
						$sql1='SELECT * FROM DETALII WHERE ID_DETALII=:valoare';
